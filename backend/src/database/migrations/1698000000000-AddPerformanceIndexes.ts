...
export class AddPerformanceIndexes1698000000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS "idx_courses_organization_status" ON "courses" ("organization_id", "status");
    `);
    ...
    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS "idx_users_fulltext" ON "users" USING gin(to_tsvector('english', first_name || ' ' || last_name || ' ' || COALESCE(email, '')));
    `);
  }
