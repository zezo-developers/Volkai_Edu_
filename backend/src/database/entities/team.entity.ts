...
export class Team {
  ...
  get goalsCompletionRate(): number {
    if (!this.metrics.performance || this.metrics.performance.totalGoals === 0)
      return 0;
    return (
      (this.metrics.performance.goalsCompleted /
        this.metrics.performance.totalGoals) *
      100
    );
  }
  ...
  calculateProductivityScore(): number {
    ...
    const performanceScore = this.averagePerformanceRating * 20;
  ...
  generateTeamReport(): any {
    return {
      ...
      goals: {
        ...
        completed:
          this.settings.goals?.filter((g) => g.status === 'completed')
            .length || 0,
      },
